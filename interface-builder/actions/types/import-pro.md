...
Чтобы ограничить использование системных ресурсов при одновременном выполнении асинхронных задач, можно управлять конкурентностью и распределением ресурсов с помощью следующих переменных окружения:

- `ASYNC_TASK_MAX_CONCURRENCY`

Ограничивает количество асинхронных задач, которые могут выполняться одновременно.
Значение по умолчанию: 3.

- `ASYNC_TASK_CONCURRENCY_MODE`

Задаёт режим управления конкурентностью.
Доступные значения: `app`, `process`.
Значение по умолчанию: `app`.

- При значении `app` максимальное число асинхронных задач, выполняющихся одновременно, для каждого подприложения ограничивается значением, заданным в `ASYNC_TASK_MAX_CONCURRENCY`.
- При значении `process` общее число одновременных асинхронных задач для всех подприложений внутри одного процесса не должно превышать значение `ASYNC_TASK_MAX_CONCURRENCY`.

- `ASYNC_TASK_WORKER_MAX_OLD` и `ASYNC_TASK_WORKER_MAX_YOUNG`

Ограничивают максимальный объём памяти кучи, которую рабочие потоки могут выделять для задач:
- Старое поколение (МБ): `ASYNC_TASK_WORKER_MAX_OLD`
- Молодое поколение (МБ): `ASYNC_TASK_WORKER_MAX_YOUNG`
