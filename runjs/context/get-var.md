...
**Асинхронно** читает переменную из текущего контекста выполнения. Источники переменных те же, что и для SQL и шаблонов `{{ctx.xxx}}`: текущий пользователь, текущая запись, параметры представления, контекст всплывающего окна и т. д.
