...
Удаляет слушатель, зарегистрированный через `ctx.on(eventName, handler)`. Используется вместе с [ctx.on](./on.md); отписывайтесь вовремя, чтобы избежать утечек памяти и повторных срабатываний.
