...
В рабочем процессе согласования нужен специальный узел «Согласование» для настройки логики обработки инициированного согласования утверждающими (одобрить, отклонить или вернуть). Узел «Согласование» можно использовать только в процессах согласования.

:::info{title=Совет}
Отличие от обычного узла «Ручной»: обычный «Ручной» подходит для более общих сценариев, например ручного ввода данных или ручного решения о продолжении процесса в разных типах рабочих процессов. Узел «Согласование» — специализированный узел только для процессов согласования: он обрабатывает только данные инициированного согласования и не используется в других рабочих процессах.
:::
...
![Режим ветвления](https://static-docs.nocobase.com/57dc6a8907f3bb02fb28c354c241e4e5.png)
    ...
    Если для узла настроено действие «Вернуть», будет создана ветвь «Вернуть». После выполнения ветви «Вернуть» текущий процесс принудительно завершается.
...
«Переназначить» и «Добавить подписанта» — нефинальные действия, которые используются для динамической корректировки утверждающих в процессе согласования. Действие «Переназначить» передает задачу согласования текущего пользователя другому пользователю. Действие «Добавить подписанта» добавляет утверждающего до или после текущего утверждающего, и новый участник продолжает процесс согласования вместе с остальными.
...
Журнал обработки согласования — это массив, содержащий записи обработки всех утверждающих в этом узле. Каждая запись включает следующие поля:

| Поле | Тип | Описание |
| --------- | ------ | ----------------------------------------- |
| id | число | Уникальный идентификатор записи обработки |
| userId | число | Идентификатор пользователя, обработавшего эту запись |
| status | число | Статус обработки |
| comment | строка | Комментарий в момент обработки |
| updatedAt | строка | Время обновления записи обработки |

Эти поля можно использовать как переменные в последующих узлах по необходимости.
