...
Используется для ограничения максимального числа итераций узлов цикла по ролям операций, чтобы избежать бесконечных циклов из-за ошибок конфигурации. По умолчанию ограничений нет, лимит можно настроить этой переменной окружения.

```ini
# Ограничение максимум до 100 итераций
WORKFLOW_LOOP_LIMIT=100
```

Если задан лимит, при выполнении узла «Цикл» и превышении этого значения будет сразу возвращена ошибка, и процесс завершится. В результате узла появится поле `exceeded` со значением «истина».
