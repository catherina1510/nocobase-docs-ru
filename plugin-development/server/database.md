...
- **Коллекция**: определяет структуру таблицы данных.
- **Модель**: соответствует моделям ORM (обычно управляемым Sequelize).
- **Репозиторий**: уровень репозитория, который инкапсулирует логику доступа к данным и обеспечивает методы работы более высокого уровня.
- **FieldType**: типы полей.
- **FilterOperator**: операторы, используемые для фильтрации.
- **Событие**: события жизненного цикла и события базы данных.
