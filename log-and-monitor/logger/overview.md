...
- Чтобы лучше отслеживать, что сделал пользователь и к чему это привело, параметры запроса и ответы сохраняются как метаданные.
  Это частично пересекается с логами запросов, но не совпадает полностью — например, логи запросов обычно не содержат полное тело запроса.
- Параметры запроса и ответы **не эквивалентны** снимкам данных.
  Они показывают, какие операции выполнялись, но не точные данные до изменения, поэтому не подходят для контроля версий или восстановления данных после ошибочных операций.
